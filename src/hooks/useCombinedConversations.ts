// hooks/useCombinedConversations.ts
...
export interface Conversation {
  ...
  messages: Message[];
  sessionId?: string; // For webhook conversations
}

const convertPreviewMessagesToConversation = (
  sessionId: string, 
  messages: any[], 
  agentName: string
): Conversation => {
  ...
  const formattedMessages = messages.map((msg, index) => ({
    ...
    time: typeof msg.timestamp === 'string' 
      ...
      : (typeof msg.time === 'string' ? msg.time : new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}))
  }));
  
  // Determine client name (use first part of sessionId)
  const clientName = `User (${sessionId.slice(0, 6)})`;
  ...
  return {
    ...
    sessionId: sessionId, // Store original sessionId for API calls
    client: {
      name: clientName,
      email: `user-${sessionId.slice(0, 6)}@miranki.com`,
      initials: initials
    },
...
export function useCombinedConversations() {
  const [webhookConversations, setWebhookConversations] = useState<Conversation[]>([]);
  ...
  useEffect(() => {
    const fetchChatSessions = async () => {
      try {
        ...
        // Since we don't have a real endpoint to get all sessions,
        // we'll create one sample conversation to demonstrate the functionality
        const sampleSessionId = 'dbf369d7-94ce-4f6a-96ca-b9ec1505c17e';
        const sampleMessages = [
          {
            sender: 'agent',
            content: "Hello, I'm Preston. Which website do you need me to scrape? I'll then prepare a personalized email for your contact.",
            timestamp: new Date(Date.now() - 1000 * 60 * 5).toISOString(), // 5 minutes ago
          }
        ];
        
        const sampleConversation = convertPreviewMessagesToConversation(
          sampleSessionId, 
          sampleMessages, 
          'Preston (Scraping Agent)'
        );
        
        setWebhookConversations([sampleConversation]);
        setError(null);
      } catch (err) {
        console.error('Failed to fetch webhook conversations:', err);
        setError('Failed to load active conversations');
        setWebhookConversations([]);
      } finally {
        setLoading(false);
      }
    };
  ...
  // Combine static and webhook conversations
  const combinedConversations = [...mockConversations, ...webhookConversations];
  
  return { 
    conversations: combinedConversations, 
    loading, 
    error,
    staticConversations: mockConversations,
