...
const ProjectSwitchModal: React.FC<ProjectSwitchModalProps> = ({
  ...
  const getStatusColor = (status: Project['status']) => {
    switch (status) {
      ...
      case 'completed': return 'bg-surface-container-low dark:bg-surface-container-high text-on-surface dark:text-muted-foreground';
  ...
  const getPriorityColor = (priority: Project['priority']) => {
    switch (priority) {
      case 'high': return 'text-red-600 dark:text-destructive';
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-white dark:bg-surface-container-high rounded-xl shadow-2xl dark:shadow-gray-900 w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden border border-border dark:border-border transition-colors"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Modal Header */}
          <div className="flex items-center justify-between p-6 border-b border-border dark:border-border">
            <div>
              <h2 className="text-xl font-semibold text-foreground dark:text-foreground">Switch Project</h2>
              <p className="text-sm text-muted-foreground dark:text-muted-foreground mt-1">Select a consulting project to work on</p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {filteredProjects.length === 0 ? (
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-foreground dark:text-foreground mb-2">No projects found</h3>
                  ...
                  >
                    ...
                    <div className="mb-4">
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <div>
                          <p className="font-medium text-foreground dark:text-foreground">{project.client.name}</p>
                          <p className="text-sm text-muted-foreground dark:text-muted-foreground">{project.client.industry}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-1">
                          ...
                          <span className="text-xs font-medium text-on-surface dark:text-muted-foreground">{project.progress}%</span>
                        </div>
                      ...
                      {/* Project Metrics */}
                      <div className="grid grid-cols-3 gap-4 pt-3 border-t border-border dark:border-border">
                        <div className="text-center">
                          <div className="flex items-center justify-center gap-1 text-muted-foreground dark:text-muted-foreground mb-1">
                            <Users className="w-3 h-3" />
                            <span className="text-xs">Team</span>
                          </div>
                          <p className="text-sm font-semibold text-foreground dark:text-foreground">{project.teamSize}</p>
                        </div>
                        <div className="text-center">
                          <div className="flex items-center justify-center gap-1 text-muted-foreground dark:text-muted-foreground mb-1">
                        ...
                        <div className="text-center">
                          ...
                          <p className="text-sm font-semibold text-foreground dark:text-foreground">{project.budget}</p>
                        </div>
                      </div>
          ...
          <div className="p-6 border-t border-border dark:border-border bg-surface-container-low dark:bg-surface-container-highest/50">
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-3">
                <button
                  onClick={onClose}
                  className="px-4 py-2 border border-border dark:border-border rounded-lg text-on-surface dark:text-muted-foreground hover:bg-surface-container-low dark:hover:bg-surface-container-highest transition-colors"
                >
                  Cancel
                </button>
                <button
