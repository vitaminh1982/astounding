...
const ChatHeader: React.FC<ChatHeaderProps> = ({ 
  ...
  const renderBadge = (count?: number) => {
    ...
    return (
      <span 
        className="ml-2 px-2 py-0.5 text-xs font-semibold bg-destructive text-white rounded-full min-w-[20px] text-center"
  ...
  const renderTab = (tab: TabConfig) => {
    ...
    return (
      ...
      >
        ...
        {isActive && (
          <span 
            className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary dark:bg-primary animate-slide-in"
            aria-hidden="true"
          />
        )}
        ...
        {loading && isActive && (
          <span 
            className="absolute inset-0 bg-white/50 dark:bg-surface-container-high/50 flex items-center justify-center"
            aria-label="Loading"
          >
            <svg 
  ...
  return (
    <div className="border-b border-border dark:border-border bg-white dark:bg-surface-container-high transition-colors">
      <nav 
        role="tablist" 
      ...
      {/* Loading bar */}
      {loading && (
        <div className="h-0.5 w-full bg-surface-container dark:bg-surface-container-highest overflow-hidden">
          <div className="h-full bg-primary dark:bg-primary animate-progress" />
        </div>
      )}
    </div>
  );
