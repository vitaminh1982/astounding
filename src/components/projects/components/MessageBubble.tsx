...
const MessageBubble: React.FC<MessageBubbleProps> = ({ 
  ...
  const renderFeedbackForm = () => {
    ...
    return (
      <div 
        className="mt-4 p-4 bg-white rounded-lg border border-orange-200 shadow-sm"
        role="form"
        aria-label="Feedback form"
      >
        <div className="flex items-start gap-2 mb-3">
          <AlertCircle className="w-4 h-4 text-orange-500 mt-0.5 flex-shrink-0" aria-hidden="true" />
        ...
        <div className="flex gap-2 mt-3">
          <button
            ...
            disabled={feedbackLoading}
            className="flex-1 px-4 py-2 bg-orange-500 text-white text-sm font-medium rounded-lg hover:bg-orange-600 active:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
          >
            {feedbackLoading ? (
              <>
      ...
      <div className={`max-w-[70%] ${isUser ? 'max-w-[80%]' : ''}`}>
          ...
          {message.attachments && message.attachments.length > 0 && (
            <div className="mt-3 space-y-2">
              {message.attachments.map((att) => (
                ...
                >
                  <Paperclip className={`w-3.5 h-3.5 ${isUser ? 'text-indigo-200' : 'text-gray-500'}`} />
                  <span className={`font-medium ${isUser ? 'text-white' : 'text-gray-700'}`}>
                    {att.name}
                  </span>
                  <span className={`ml-auto ${isUser ? 'text-indigo-200' : 'text-gray-400'}`}>
        ...
        <div className={`flex justify-between items-center mt-2 px-1 ${
          ...
          <div className="flex items-center gap-3">
            {/* Timestamp */}
            <span className={`text-xs ${isUser ? 'text-gray-600' : 'text-gray-500'}`}>
              {formattedTime}
            </span>
