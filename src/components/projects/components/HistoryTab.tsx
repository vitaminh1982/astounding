...
const HistoryTab: React.FC<HistoryTabProps> = ({ agents, onResumeSession }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 transition-colors">
      {/* Header */}
      <div className="bg-white dark:bg-surface-container-high border-b border-border dark:border-border shadow-sm dark:shadow-gray-900 transition-colors">
        <div className="p-6">
          <div className="flex flex-col lg:flex-row gap-6 items-start lg:items-center justify-between mb-6">
            <div>
              <h3 className="text-2xl font-bold text-foreground dark:text-foreground mb-1">Chat History</h3>
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
            <div className="flex flex-col sm:flex-row gap-3 flex-1">
              ...
              {/* Group By */}
              <div className="relative">
                <CalendarIcon className="absolute left-3 top-1/2 transform -translate-y-1/2 text-outline dark:text-muted-foreground w-4 h-4 pointer-events-none" />
                <select
            ...
            <div className="flex gap-2">
              {/* View Toggle */}
              <div className="flex bg-surface-container-low dark:bg-surface-container-highest rounded-lg p-1 transition-colors">
                <button
              ...
              {/* Export Button */}
              <button className="p-2.5 border border-border dark:border-border bg-white dark:bg-surface-container-highest rounded-lg hover:bg-surface-container-low dark:hover:bg-surface-container-highest transition-colors">
                <Download className="w-4 h-4 text-muted-foreground dark:text-muted-foreground" />
              </button>

              {/* Clear Button */}
              <button className="p-2.5 border border-border dark:border-border bg-white dark:bg-surface-container-highest rounded-lg hover:bg-surface-container-low dark:hover:bg-surface-container-highest transition-colors">
                <Trash2 className="w-4 h-4 text-muted-foreground dark:text-muted-foreground" />
              </button>
            </div>
          </div>
        ...
        {Object.keys(groupedSessions).length === 0 ? (
          <div className="text-center py-16">
            <div className="w-24 h-24 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-600 rounded-full flex items-center justify-center mx-auto mb-6 transition-colors">
              <Clock className="w-12 h-12 text-outline dark:text-muted-foreground" />
            </div>
            <h3 className="text-xl font-semibold text-foreground dark:text-foreground mb-2">No chat history found</h3>
              ...
              <div key={groupKey}>
                ...
                <div className="sticky top-0 z-10 bg-gradient-to-r from-gray-100 to-transparent dark:from-gray-700 dark:to-transparent backdrop-blur-sm py-3 px-4 rounded-lg mb-4 border-l-4 border-primary-green dark:border-teal-500 transition-colors">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="bg-white dark:bg-surface-container-highest text-muted-foreground dark:text-muted-foreground px-3 py-1 rounded-full text-xs font-medium shadow-sm dark:shadow-gray-900 transition-colors">
