    ...
    <div className="bg-white dark:bg-surface-container-high rounded-lg shadow-sm dark:shadow-gray-900 border border-border dark:border-border p-6 transition-colors">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative group">
            <button 
              className="p-2 bg-surface-container-low dark:bg-surface-container-highest rounded-lg hover:bg-surface-container dark:hover:bg-surface-container-highest transition-colors focus:outline-none focus:ring-2 focus:ring-ring dark:focus:ring-ring focus:ring-offset-2 dark:focus:ring-offset-gray-800"
              onClick={() => onExport('csv')}
            >
              <Download className="w-5 h-5 text-muted-foreground dark:text-muted-foreground" />
            </button>
            <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-surface-container-high border border-border dark:border-border rounded-md shadow-lg dark:shadow-gray-900 py-1 z-10 hidden group-hover:block transition-colors">
              <button 
                className="block w-full text-left px-4 py-2 text-sm text-on-surface dark:text-on-surface-variant hover:bg-surface-container-low dark:hover:bg-surface-container-highest transition-colors"
                onClick={() => onExport('csv')}
              >
                Export as CSV
              </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div className="bg-surface-container-low dark:bg-surface-container-highest border border-border dark:border-border rounded-lg p-4 transition-colors">
          <p className="text-sm text-muted-foreground dark:text-muted-foreground">Credits Used</p>
          <div className="flex items-end justify-between">
            <p className="text-2xl font-bold text-foreground dark:text-foreground">{currentMonth.credits.toLocaleString()}</p>
            <div className={`flex items-center text-sm ${creditsChange.isIncrease ? 'text-destructive dark:text-destructive' : 'text-green-500 dark:text-green-400'}`}>
        ...
        <div className="bg-surface-container-low dark:bg-surface-container-highest border border-border dark:border-border rounded-lg p-4 transition-colors">
          ...
          <div className="flex items-end justify-between">
            <p className="text-2xl font-bold text-foreground dark:text-foreground">{currentMonth.messages.toLocaleString()}</p>
            <div className={`flex items-center text-sm ${messagesChange.isIncrease ? 'text-destructive dark:text-destructive' : 'text-green-500 dark:text-green-400'}`}>
        ...
        <div className="bg-surface-container-low dark:bg-surface-container-highest border border-border dark:border-border rounded-lg p-4 transition-colors">
          ...
          <div className="flex items-end justify-between">
            <p className="text-2xl font-bold text-foreground dark:text-foreground">{currentMonth.conversations.toLocaleString()}</p>
            <div className={`flex items-center text-sm ${conversationsChange.isIncrease ? 'text-destructive dark:text-destructive' : 'text-green-500 dark:text-green-400'}`}>
      ...
      <div>
        <h4 className="text-sm font-medium text-on-surface dark:text-on-surface-variant mb-4">Monthly Comparison</h4>
        <div className="h-64">
          <div className="h-full flex items-end justify-between">
            {history.map((month, index) => (
              <div key={index} className="flex flex-col items-center flex-1">
                ...
                <div className="text-xs text-muted-foreground dark:text-muted-foreground mt-2">{month.month.substring(0, 3)}</div>
              </div>
            ))}
          </div>
        </div>
