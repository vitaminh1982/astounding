    ...
    <div className="bg-white dark:bg-surface-container-high rounded-lg shadow-sm dark:shadow-gray-900 border border-border dark:border-border p-6 transition-colors">
      ...
      {isEditing ? (
        <div className="space-y-6">
          <div>
            <h4 className="text-sm font-medium text-on-surface dark:text-on-surface-variant mb-4">Notification Thresholds</h4>
              ...
              {settings.thresholds.map((threshold, index) => (
                <div key={index} className="flex items-center">
                  <input
                    ...
                    className="flex-1 mr-4 h-2 bg-surface-container dark:bg-surface-container-highest rounded-lg appearance-none cursor-pointer 
                  ...
                  />
                  <div className="w-16 px-2 py-1 bg-surface-container-low dark:bg-surface-container-highest border border-border dark:border-border text-foreground dark:text-foreground rounded text-center transition-colors">
                    {threshold}%
                  </div>
                </div>
              ))}
          ...
          <div className="border-t border-border dark:border-border pt-6">
            <h4 className="text-sm font-medium text-on-surface dark:text-on-surface-variant mb-4">Email Notifications</h4>
            <div className="flex items-center mb-4">
              <input
                ...
                className="h-4 w-4 text-primary-green dark:text-primary-green focus:ring-ring dark:focus:ring-ring border-border dark:border-border bg-white dark:bg-surface-container-highest rounded transition-colors"
              />
              <label htmlFor="email-notifications" className="ml-2 block text-sm text-on-surface dark:text-on-surface-variant">
            ...
            {settings.emailNotifications && (
              <div>
                <label htmlFor="email-address" className="block text-sm font-medium text-on-surface dark:text-on-surface-variant mb-1">
                  Email Address
                </label>
                <div className="flex">
                  <div className="relative flex-grow">
                    <Mail className="absolute left-3 top-1/2 transform -translate-y-1/2 text-outline dark:text-muted-foreground w-5 h-5" />
                    <input
                      type="email"
                      id="email-address"
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-border dark:border-border">
            <button
              ...
              className="px-4 py-2 border border-border dark:border-border rounded-md text-on-surface dark:text-on-surface-variant bg-white dark:bg-surface-container-highest hover:bg-surface-container-low dark:hover:bg-surface-container-highest transition-colors focus:outline-none focus:ring-2 focus:ring-ring dark:focus:ring-ring focus:ring-offset-2 dark:focus:ring-offset-gray-800"
            >
              Cancel
            </button>
            <button
        ...
        <div className="space-y-6">
          <div>
            <h4 className="text-sm font-medium text-on-surface dark:text-on-surface-variant mb-4">Current Notification Thresholds</h4>
            <div className="flex flex-wrap gap-2 mb-4">
              {settings.thresholds.map((threshold, index) => (
                <div key={index} className="px-3 py-1 bg-surface-container-low dark:bg-surface-container-highest border border-border dark:border-border text-on-surface dark:text-on-surface-variant rounded-full text-sm transition-colors">
                  {threshold}%
                </div>
              ))}
            </div>
            ...
            <div className="bg-surface-container-low dark:bg-surface-container-highest border border-border dark:border-border rounded-lg p-4 transition-colors">
              <div className="flex items-center">
                ...
                <div>
                  <p className="font-medium text-foreground dark:text-foreground">Email Notifications</p>
