    ...
    <div className="space-y-4 relative">
      ...
      {integrations.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {integrations.map((integration) => (
            <div key={integration} className="flex items-center justify-between p-3 border border-border dark:border-border bg-white dark:bg-surface-container-highest rounded-lg transition-colors">
              <div className="flex items-center gap-3">
                ...
                <span className="text-foreground dark:text-foreground">{integration}</span>
              </div>
              <div className="flex items-center gap-2">
                ...
                <button 
                  ...
                  className="p-1 text-outline dark:text-muted-foreground hover:text-destructive dark:hover:text-destructive rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:focus:ring-offset-gray-700"
        ...
        <div className="text-center py-8 border border-border dark:border-border rounded-lg bg-surface-container-low dark:bg-surface-container-highest transition-colors">
          <Link2 className="w-10 h-10 text-outline dark:text-muted-foreground mx-auto mb-3" />
          <p className="text-muted-foreground dark:text-muted-foreground">No integrations connected yet</p>
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 dark:bg-black dark:bg-opacity-70 flex items-center justify-center z-50 p-4 transition-colors">
          <div className="bg-white dark:bg-surface-container-high border border-border dark:border-border rounded-lg shadow-xl dark:shadow-gray-900 w-full max-w-md max-h-[80vh] flex flex-col transition-colors">
            <div className="p-4 border-b border-border dark:border-border">
              <h3 className="text-lg font-semibold text-foreground dark:text-foreground">Add Integration</h3>
              <p className="text-sm text-muted-foreground dark:text-muted-foreground mt-1">Connect your agent with tools and services</p>
            </div>
            
            <div className="p-4 border-b border-border dark:border-border">
              <div className="relative">
                <Search className="absolute left-3 top-2.5 w-4 h-4 text-outline dark:text-muted-foreground" />
                <input
                  type="text"
                  placeholder="Search integrations..."
                  className="w-full pl-10 pr-4 py-2 border border-border dark:border-border bg-white dark:bg-surface-container-highest text-foreground dark:text-foreground rounded-lg focus:outline-none focus:ring-2 focus:ring-ring dark:focus:ring-ring transition-colors"
                  value={searchTerm}
              ...
              {filteredIntegrations.length > 0 ? (
                <div className="divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredIntegrations.map((integration) => (
                    <div key={integration.id} className="flex items-center justify-between p-4 hover:bg-surface-container-low dark:hover:bg-surface-container-highest transition-colors">
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-surface-container-low dark:bg-surface-container-highest rounded-lg transition-colors">
                          <Link2 className="w-5 h-5 text-muted-foreground dark:text-muted-foreground" />
                        </div>
                        <div>
                          <p className="font-medium text-foreground dark:text-foreground">{integration.name}</p>
                          <p className="text-xs text-muted-foreground dark:text-muted-foreground">{integration.category}</p>
                        </div>
                      </div>
                      <button
