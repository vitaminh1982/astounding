...
const SendplexNavItem = memo(({
  ...
  return (
    <button
      ...
      title={label}
      className={[
        'w-full flex flex-col items-center gap-1 py-2 px-1 rounded-xl transition-colors focus:outline-none',
        isActive
          ? 'bg-white/60 dark:bg-white/10 text-gray-900 dark:text-gray-100 shadow-sm'
...
const NotificationItem = memo(({ notification, onMarkAsRead, index }: {
  ...
  return (
    <div
      ...
      tabIndex={0}
      onKeyDown={(e) => {
        if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onMarkAsRead(); }
      }}
    >
...
const Sidebar = ({
  ...
  useEffect(() => {
    if (
      isAnyAgentsActive() || currentPage === 'agents' ||
      currentPage === 'templates' || currentPage === 'prompts' ||
      currentPage === 'documents' || currentPage === 'integrations' || currentPage === 'workflows'
    ) {
      setWorkspaceMode('studio');
      setIsAgentsMenuOpen(true);
      setLastStudioPage(currentPage);
    } else if (
      isAnyGovernanceActive() || isAnyOrchestrationActive() ||
      currentPage === 'governance' || currentPage === 'orchestration' ||
      currentPage === 'policy-management' || currentPage === 'audit-compliance' ||
      currentPage === 'risk-management' || currentPage === 'performance-analytics' ||
      currentPage === 'agent-configuration' || currentPage === 'agent-management' ||
      currentPage === 'workflow-management' || currentPage === 'monitoring-analytics' ||
      currentPage === 'resource-management' || currentPage === 'collaboration'
    ) {
  ...
  const handleWorkspaceModeChange = (mode: 'work' | 'studio' | 'govern') => {
    ...
    if (mode === 'work') handleNav(lastWorkPage);
    else if (mode === 'studio') handleNav(lastStudioPage);
    else if (mode === 'govern') handleNav(lastGovernPage);
  };
  ...
  return (
    ...
    >
      ...
      {/* ══════════════════════════════════════════════════════════
          #sendplex-nav — narrow left panel with brand gradient
          ══════════════════════════════════════════════════════════ */}
      <div
        id="sendplex-nav"
        className="w-16 flex-shrink-0 flex flex-col sendplex-nav-bg rounded-2xl"
      >
        {/* Logo */}
        <div className="flex items-center justify-center h-14 px-2 flex-shrink-0">
          <button
        ...
        {/* Nav items */}
        <div className="flex-1 flex flex-col items-center gap-1 px-2 pt-1 overflow-y-auto">
          <SendplexNavItem
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-3">
                  ...
                  <div onMouseEnter={handleHoverReset}>
                    <button
                      onClick={() => { handleNav('usage'); setIsProfileMenuOpen(false); }}
          ...
          >
            <div className="relative">
              ...
              {(() => {
                const pill = getPlanPillText(activeWorkspace.plan);
                return pill ? (
                  <span className="absolute -bottom-2 left-1/2 -translate-x-1/2 bg-black/80 dark:bg-white/80 text-white dark:text-black text-[8px] font-semibold px-1.5 py-0.5 rounded-full whitespace-nowrap">
                    {pill}
                  </span>
                ) : null;
              })()}
      ...
      {/* ══════════════════════════════════════════════════════════
          #workspace-nav — main sidebar content, collapses on desktop
          ══════════════════════════════════════════════════════════ */}
      <div
        id="workspace-nav"
        className={[
          'flex-col w-48 flex-shrink-0 glass-sidebar rounded-2xl overflow-hidden',
          isExpanded ? 'flex' : 'flex lg:hidden',
        ].join(' ')}
      >
        {/* PanelLeft toggle */}
        <div className="flex items-center justify-between h-14 px-3 flex-shrink-0">
          <span className="text-xs font-semibold text-gray-400 dark:text-gray-500 truncate pr-2">
            {activeWorkspace.name}
          </span>
          <button
            onClick={onToggleExpand}
            className="hidden lg:flex p-1.5 rounded-md text-[#888888] dark:text-[#bbbbbb] hover:bg-white/40 dark:hover:bg-white/10 hover:text-black dark:hover:text-white transition-colors flex-shrink-0"
        ...
        {/* workspace-selector */}
        <div id="workspace-selector" className="px-2 mb-2 flex-shrink-0">
          <NavWorkspaceSwitcher />
        </div>

        {/* mode-switcher */}
        <div id="mode-switcher" className="p-1 mx-2 mb-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl flex-shrink-0 relative flex">
          <button
