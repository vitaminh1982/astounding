...
const HumanAICollaborationFramework: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-lg p-6 shadow-sm" aria-label="Human-AI Collaboration Framework diagram showing how humans and AI systems work together, with human strengths on the left, AI capabilities on the right, and collaboration zones in the center">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div className="w-full md:w-1/3 bg-blue-50 rounded-lg p-4">
          <div className="text-center mb-4">
            ...
            <h3 className="text-lg font-semibold text-blue-800">Human Strengths</h3>
          </div>
          
          <div className="space-y-3">
            {humanStrengths.map(strength => (
              <div key={strength.id} className="flex items-center bg-white p-2 rounded-lg shadow-sm">
                <div className="p-1.5 bg-blue-100 rounded-full mr-2">
                  {React.cloneElement(strength.icon, { className: "text-blue-600" })}
                </div>
        ...
        <div className="w-full md:w-1/3 relative">
          ...
          <div className="relative">
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-48 h-48 rounded-full bg-gradient-to-r from-blue-200 to-indigo-200 opacity-20"></div>
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-40 h-40 rounded-full bg-gradient-to-r from-blue-300 to-indigo-300 opacity-20"></div>
            
            {/* Central icon */}
            <div className="relative flex flex-col items-center">
              <div className="w-20 h-20 rounded-full bg-gradient-to-r from-blue-500 to-indigo-500 flex items-center justify-center shadow-lg mb-3">
                <RefreshCw className="w-10 h-10 text-white" />
              </div>
              <h3 className="text-lg font-bold text-gray-800 text-center mb-4">Collaboration Zones</h3>
              
              {/* Bidirectional arrows */}
              <div className="hidden md:block absolute left-0 top-1/2 transform -translate-y-1/2 -translate-x-full">
                <div className="flex items-center">
                  <div className="w-8 h-0.5 bg-blue-400"></div>
                  <div className="w-0 h-0 border-t-4 border-b-4 border-r-8 border-t-transparent border-b-transparent border-r-blue-400"></div>
                </div>
              </div>
              <div className="hidden md:block absolute right-0 top-1/2 transform -translate-y-1/2 translate-x-full">
                <div className="flex items-center">
                  <div className="w-0 h-0 border-t-4 border-b-4 border-l-8 border-t-transparent border-b-transparent border-l-indigo-400"></div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-4">
              {collaborationZones.map(zone => (
                <div key={zone.id} className="bg-gradient-to-r from-blue-50 to-indigo-50 p-2 rounded-lg shadow-sm">
                  <div className="flex items-center">
                    <div className="p-1.5 bg-white rounded-full mr-2 shadow-sm">
                      {React.cloneElement(zone.icon, { className: "text-indigo-600 w-3.5 h-3.5" })}
                    </div>
        ...
        <div className="w-full md:w-1/3 bg-indigo-50 rounded-lg p-4">
          <div className="text-center mb-4">
            <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-indigo-100 mb-2">
              <Brain className="w-6 h-6 text-indigo-600" />
            </div>
            <h3 className="text-lg font-semibold text-indigo-800">AI Capabilities</h3>
          </div>
          
          <div className="space-y-3">
            {aiCapabilities.map(capability => (
              <div key={capability.id} className="flex items-center bg-white p-2 rounded-lg shadow-sm">
                <div className="p-1.5 bg-indigo-100 rounded-full mr-2">
                  {React.cloneElement(capability.icon, { className: "text-indigo-600" })}
                </div>
